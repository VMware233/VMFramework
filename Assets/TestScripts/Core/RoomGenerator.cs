...
public class RoomGenerationInfo : ITreeNode<RoomGenerationInfo>
{
    ...
    private Dictionary<FourTypesDirection2D, RoomGenerationInfo> _childrenRoomDict =
        new();
    ...
    public RoomGenerationInfo(FourTypesDirection2D enterDirection,
        RoomGenerationInfo parentRoomInfo)
    {
    ...
    private void AddChildRoom(FourTypesDirection2D direction,
        RoomGenerationInfo roomGenerationInfo)
    {
        if (direction == enterDirection)
            throw new System.Exception("不能在入口方向添加子房间");
    ...
    [ShowInInspector]
    IEnumerable<RoomGenerationInfo> IChildrenProvider<RoomGenerationInfo>.
        GetChildren() =>
